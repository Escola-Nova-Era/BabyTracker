...
import com.escolanovaeratech.babytracker.theme.ChartBar2
...
@Composable
fun SleepingCard(
    sleepingDataList: List<SleepData>,
    averageHours: Float,
    modifier: Modifier = Modifier
) {
    val avgHoursInt = averageHours.toInt()
    val avgMinutes = ((averageHours - avgHoursInt) * 60).toInt()
    val formattedAvg = if (avgMinutes > 0) "${avgHoursInt}h ${avgMinutes}m" else "${avgHoursInt}h"
    ...
    Card(
        ...
        Box(
            ...
            Column(
                ...
                // Gráfico de Barras de Sono
                SleepingBarChart(
                    data = sleepingDataList,
                    modifier = Modifier
                        .fillMaxWidth()
                        .height(160.dp)
                )
                ...
                // Summary Inferior
                Text(
                    text = stringResource(R.string.summary_sleeping) + formattedAvg + stringResource(
                        R.string.summary_sleeping_2
                    ),
...
@Composable
fun SleepingBarChart(
    data: List<SleepData>,
    modifier: Modifier = Modifier,
    barColor: Color = ChartBar2,
    maxHours: Float = 24f
) {
    val yAxisSteps = listOf(24, 18, 12, 6, 0)

    Row(modifier = modifier) {
        // Eixo Y (Valores de horas na esquerda)
        Column(
            modifier = Modifier
                .fillMaxHeight()
        ...
        Column(modifier = Modifier.weight(1f)) {
            Canvas(
                ...
                data.forEachIndexed { index, item ->
                    ...
                    val barHeight = (item.hours.coerceAtMost(maxHours) / maxHours) * height
...
fun SleepingCardPreview() {
    ...
    BabyTrackerTheme {
        SleepingCard(
            ...
            averageHours = sampleSleepData.map { it.hours }.average().toFloat()
        )
    }
}